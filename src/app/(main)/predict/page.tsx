...
export default function PredictPage() {
  ...
  const handlePriceUpdate = useCallback((update: PriceUpdate) => {
    // 가격 업데이트 플래그 설정
    setPriceUpdated((prev) => ({ ...prev, [update.stock_code]: true }));
  ...
  const renderPredictionItem = (item: PredictionItem) => {
    ...
    const getPriceChangeClass = () => {
      if (!isUpdated || item.current_price === null) return "";
      if (item.current_price > item.stock_open) return "bg-red-100 dark:bg-red-900/30";
      if (item.current_price < item.stock_open) return "bg-blue-100 dark:bg-blue-900/30";
      return "";
    };
